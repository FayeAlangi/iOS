...
// TODO: Import PhotosUI
    ...
    private func updateUI() {
        ...
        if task.image != nil {
            completedImage = UIImage(systemName: "circle.inset.filled")
        }else{
            completedImage = UIImage(systemName: "circle")
        }
    ...
    func updateMapView() {
        ...
        let coordinate = imageLocation.toCLLocationCoordinate2D
...
extension TaskDetailViewController: PHPickerViewControllerDelegate {
    ...
    func picker(_ picker: PHPickerViewController, didFinishPicking results: [PHPickerResult]) {
        ...
        provider.loadObject(ofClass: UIImage.self) { [weak self] object, error in
            ...
            self?.task.image = imageFile
           
            self?.task.location = try? ParseGeoPoint(location: location)
                self?.task.save{ [weak self] result in
...
extension TaskDetailViewController: MKMapViewDelegate {
    ...
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        annotationView.configure(with: self.pickedImage)
